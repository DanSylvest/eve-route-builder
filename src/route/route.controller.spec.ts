...
describe('RouteController', () => {
  ...
  it('should be success', () => {
    expect(controller.route(AMARR, PERIMETER, 'secure')).toEqual([
  ...
  it('should be success if insecure', () => {
    expect(controller.route(AMARR, PERIMETER, 'insecure')).toEqual([
  ...
  it('should be success if shortest', () => {
    expect(controller.route(AMARR, PERIMETER, 'shortest')).toEqual([
  ...
  it('should be with additional connections', () => {
    const additional = [[BHIZHEBA, J212812]];
    expect(controller.route(J212812, AMARR, 'secure', additional)).toEqual([31001180, 30002282, 30002187]);
  });

  it('should be if no connection', () => {
    expect(controller.route(J212812, AMARR, 'secure')).toEqual([]);
  });

  it('should returns error', () => {
    expect(controller.route(100, AMARR, 'secure')).toEqual('Origin solar system - 100 is not exists.');
    expect(controller.route(AMARR, 100, 'secure')).toEqual('Destination solar system - 100 is not exists.');
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    expect(controller.route(AMARR, PERIMETER, 'xxx')).toEqual(
